...
u8 cursor;
int fb1[1 * 1024 * 1024];   // Array
int fb2[1 * 1024 * 1024];   // Array
int volatile *fb;           // Pointer
unsigned char *font;
int row, col;
int frame_buffer;
...
// remove debug *(volatile unsigned int *)(0x10120024) = 0xc; // enable interrupts
}
...
void vid_handler() {
    // Tell device that we are handling the interrupts, otherwise it will keep interrupting
    *(volatile unsigned int *) (0x10120028) = 0;  // Clear last interrupt
    *(volatile unsigned int *) (0x1012001C) = 0;  // Turn off interrupts

    // Swap the frame buffer to the back buffer
    // fb = pointer inside vid ctlr
    // Gets filled with stuff from Draw_all() --> draws Pacman and ghosts, gets them to move
    // The back buffer gets cleared there, but not the front buffer
    *(volatile unsigned int *) (0x10120010) = fb;   // Put the back buffer into register in vid ctlr

    // To swap frame buffer, first determine:
    // Is fb currently pointing to fb1? If so, make fb point to fb2
    if (fb == fb1) {
        fb = fb2;
    } else {
        fb = fb1;
